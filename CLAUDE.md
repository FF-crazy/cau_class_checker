...
无头 HTTP 是一条路，WebView 是另一条（`ui/ManualCheckInScreen.kt`）——
服务端改了、要人机交互时，后者还能用。慢，但只要能上网就行。

⚠️ **`android.webkit.CookieManager` 是全应用共享的单例**，没有「每个账号一个罐子」
这回事。所以每换一个账号就**整幅重建 WebView**（`key(username)`），
重建前 `removeAllCookies` 再注入这一个账号的 Cookie。复用同一幅会让上一个账号的
会话残留下来 —— 那正好毁掉多账号的前提。离开这个页面时也要再清一次，
别把最后一个账号的会话留在全局存储里。

**自动前进只在「这条路确实走完了」的时候**：签到成功 / 已签到 / 会话失效。
「码不对」「LastVisit 超时」都是**可以重来的**（「重新加载」就是干这个的），
自动跳过等于替用户放弃一次可能成功的签到，而用户未必注意到发生过什么。

取页面文本靠 `evaluateJavascript`，但**让 JS 先 `encodeURIComponent`** 再回来
（见 `web/JsResult.kt`）—— 否则要面对一整套 JSON 转义规则（`\"`、`\n`、`\uXXXX`），
手工解迟早会漏。

## 约定与坑

- **界面文案全中文**。新增用户可见字符串照此办理。
- **应用名只从 `R.string.app_name` 取，不要在 Kotlin 里写死。**
  改名那次漏了两处硬编码（关于页一处、签到页顶栏一处），隔了几轮才被发现 ——
  因为它们不在同一处、也不报错，只是安静地显示旧名字。
  凡是要显示「早八之恨」的地方一律 `stringResource(R.string.app_name)`。
